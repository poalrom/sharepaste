...
///
/// Returns whether this was the Entry's **first** appearance in this cache. Two
/// paths now ingest the same id — the uploader caches an Entry this device
/// created, and the relay echoes it back over SSE a moment later — and the
/// caller is obliged to raise `EntryAdded` for exactly one of them. A duplicate
/// row on screen is a worse bug than the missing row this reporting exists to
/// fix, so the answer comes from the database rather than from a caller's
/// bookkeeping.
pub(crate) fn upsert_and_prune(conn: &Connection, e: NewCachedEntry<'_>, now_ms: i64) -> Result<bool, AppError> {
    let tx = conn.unchecked_transaction()?;
    // Asked inside the transaction, because `ON CONFLICT DO UPDATE` reports one
    // changed row either way and SQLite has no "was it an insert" flag.
    let already_here = tx
        .query_row(
            "SELECT 1 FROM entries_cache WHERE user_id = ?1 AND id = ?2",
